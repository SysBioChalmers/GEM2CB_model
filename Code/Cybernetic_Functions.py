...
class Cybernetic_Model(dict):
    ...
    def weight_of_method(self, exp_y, model_y, res_sq):
        ...
        if self.weight_of_method_t_f == 0:  # minimize variance
            ...
            denominators = np.mean(res_sq, axis=0) ** 2
            denominators = np.where(denominators <= 0.01, 1, denominators)
            weight_of_method = 1 / denominators / res_sq.shape[0]
            ...
            denominators = exp_y
            denominators = np.where(denominators <= 0.01, 0.1, denominators)
            res_sq_weight_of_method = res_sq / (denominators ** 2)

        elif self.weight_of_method_t_f == 3:  # model_y and exp_y , Normalization by col note : error 0/0

            denominators = exp_y.max(axis=0) - exp_y.min(axis=0)
            denominators = np.where(denominators <= 0.01, 1, denominators)
            exp_y_norm = (exp_y - exp_y.min(axis=0)) / (denominators)
            model_y_norm = (model_y - model_y.min(axis=0)) / (denominators)
        ...
        elif self.weight_of_method_t_f == 3.5:  # model_y and exp_y , Normalization by all note : error 0/0

            denominators = exp_y.max() - exp_y.min(axis=0)
        ...
        elif self.weight_of_method_t_f == 4:  #  Normalization err note : error 0/0

            denominators = res_sq.max(axis=0) - res_sq.min(axis=0)
            denominators = np.where(denominators <= 0.01, 1, denominators)
            res_sq_norm = (res_sq - res_sq.min(axis=0)) / (denominators)

            res_sq_weight_of_method = res_sq_norm

        elif self.weight_of_method_t_f == -1:  # model_y and exp_y , Normalization by col note : error 0/0
