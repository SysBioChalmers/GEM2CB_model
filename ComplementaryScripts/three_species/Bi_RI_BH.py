...
experimet_data = RI_BH_experimet_data

convert_rate_Ri = 0.877 * 5 / 71
convert_rate_Bh = 0.877 * 15.5 / 10 / 20.7

experimet_data['R.i'] = experimet_data['R.i'] * convert_rate_Ri / 1e8
experimet_data['B.h'] = experimet_data['B.h'] * convert_rate_Bh / 1e8
...
Smz_Bh = np.array([-1, 0, 0.0, 0.0887, 1.2, -2.48, 0, 0.38]).reshape(8, 1)
...
def rate_def(x, CB_model):
    ...
    # if len(Biomass_index)>1:
    #     rG = Smz[Biomass_index, :].sum(axis=0) * rM[:]  # 11: biomass index
    # else:
    #     rG = Smz[Biomass_index, :] * rM[:]  # 11: biomass index
...
simulation = sol
experiment_data_df['R.i'] = experiment_data_df['R.i'] / convert_rate_Ri
experiment_data_df['B.h'] = experiment_data_df['B.h'] / convert_rate_Bh

simulation[:, 1] = simulation[:, 1] / convert_rate_Ri
simulation[:, 3] = simulation[:, 3] / convert_rate_Bh
# simulation[:, 0] = simulation[:, 0] + 37
# biomassindexs = [3]

biomassindexs = [1, 3]  # Biomass_index
