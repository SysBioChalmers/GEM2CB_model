...
def remove_sepcical_dup(model_i_refined):
    # case: reactions id end with '_1' or '_2', compare and keep only one

    # use:model_i_refined = remove_sepcical_dup(model_i_refined)

    model = model_i_refined.copy()
    for rea in model.reactions:

        if rea.id.endswith('_1') or rea.id.endswith('_2'):

            try:
                rea2 = model_i_refined.reactions.get_by_id(re.sub('_.$', '', rea.id))
...
for sp_name in name_list:
    # sp_name = 'R_int'
    model_i_path = 'GEM_from_templates_' + sp_name + '_draft.json'
    model_i_draft_3 = cobra.io.load_json_model(model_i_path)
    model_i_refined = model_i_draft_3.copy()
    # % <general step: refine biomass and gap fill>
    Lreuteri_530.objective = "BIOMASS"
    print('Lreuteri_530 Biomass:', Lreuteri_530.optimize())

    model_i_refined.objective = "BIOMASS"
    print('model_i_refined BIOMASS:', model_i_refined.optimize())

    # <media>
    for rea in Lreuteri_530.exchanges:  # add Lrue 530 exchange reactions
        try:
            model_i_refined.reactions.get_by_id(rea.id).bounds = rea.bounds
        except:
            try:
                rea.notes['from'] = list(set(rea.notes['from'] + ['Lreuteri_530', 'exchange_reactions']))
            except:
                rea.notes['from'] = ['Lreuteri_530', 'exchange_reactions']
            model_i_refined.add_reaction(rea)
    model_i_refined.add_reaction(Lreuteri_530.reactions.get_by_id('EX_2ahbut_c'))

    model_i_refined.objective = "BIOMASS"
    print('model_i_refined changed media BIOMASS:', model_i_refined.optimize())

    # <biomass reactions>
    for rea in model_i_refined.reactions:
    ...
    for rea in Lreuteri_530.reactions:
        if '_LRE' in rea.id or '_LRE' in rea.reaction:
            try:
                model_i_refined.reactions.get_by_id(rea.id)
            except:
                model_i_refined.add_reaction(rea)
    ...
    # <DNA>
    # model_i_refined.reactions.get_by_id(
    ...
    model_i_refined.reactions.get_by_id('LTA_total').id = 'LTA_LRE'
    ...
    for rea_i in remove_list:
        try:
            model_i_refined.reactions.get_by_id(rea_i).remove_from_model()
        except:
            pass
    model_i_refined.objective = "BIOMASS"
    print('model_i_refined Biomass:', model_i_refined.optimize())

    model_i_refined.reactions.get_by_id('ATPM').bounds = Lreuteri_530.reactions.get_by_id('ATPM').bounds
    model_i_refined.reactions.get_by_id('NNDMBRT').reaction = Lreuteri_530.reactions.get_by_id('NNDMBRT').reaction
    # model_i_refined.reactions.get_by_id('MCMAT8').reaction = Lreuteri_530.reactions.get_by_id('MCMAT8').reaction
    # model_i_refined.reactions.get_by_id('HDMAT7').reaction = Lreuteri_530.reactions.get_by_id('HDMAT7').reaction

    print('gap filling')
    # gap_biomass_mets = ['LIP_LRE_c','LTAtotal_LRE_c','adeadocbl_c','btn_c','pydx5p_c']
    gap_biomass_mets = [i.id for i, j in model_i_refined.reactions.get_by_id('BIOMASS').metabolites.items() if j < 0]
    ...
    for need_id in need_fluxes.index:
        try:
            if model_i_refined.reactions.get_by_id(need_id).reaction != Lreuteri_530.reactions.get_by_id(need_id).reaction:
                print('equaction')
                print(Lreuteri_530.reactions.get_by_id(need_id),need_fluxes[need_id])
                print(model_i_refined.reactions.get_by_id(need_id))
                pass
            if need_fluxes[need_id] > model_i_refined.reactions.get_by_id(need_id).upper_bound or need_fluxes[need_id] < model_i_refined.reactions.get_by_id(need_id).lower_bound:
                print('bounds')
                print(Lreuteri_530.reactions.get_by_id(need_id),need_fluxes[need_id],Lreuteri_530.reactions.get_by_id(need_id).bounds)
                print(model_i_refined.reactions.get_by_id(need_id),need_fluxes[need_id],model_i_refined.reactions.get_by_id(need_id).bounds)
                #pass
    
        except:
            #model_i_refined.add_reaction(Lreuteri_530.reactions.get_by_id(need_id))
    
            gap_list.append(need_id)
    # %%
    for i in  gap_list:
        print(Lreuteri_530.reactions.get_by_id(i))
        model_i_refined.add_reaction(Lreuteri_530.reactions.get_by_id(i))
    ...
    model_i_refined.objective = "BIOMASS"
    model_i_refined.optimize()

    # %% <media>
    temp_model = model_i_refined.copy()
    remove_rea_ids = ['EX_2aeppn_e', 'EX_2h3mb_e', '2H3MBt',
                      ...
                      'EX_12ppd__R_e', 'EX_3hpp_e', 'EX_raffin_e',
                      # 'EX_ocdcea_e'
                      ]
    for rea_id in remove_rea_ids:
        ...
        try:
            ...
            print(rea_id, temp_model.optimize())
        except:
            print(rea_id, 'not in model')

    for rea_id in remove_rea_ids:
        try:
            model_i_refined.reactions.get_by_id(rea_id).remove_from_model()
        except:
            pass

    for rea_i in model_i_refined.reactions:
        if '_c' in rea_i.reaction and '_e' in rea_i.reaction:
            print(rea_i, rea_i.gene_reaction_rule, rea_i.notes)

    model_i_refined.objective = "BIOMASS"
    print('model_i_refined Biomass media:', model_i_refined.optimize())
    ...
    for rea_i in model_i_refined.reactions:
        try:
            if rea_i.notes['from'] == ['iNF517', 'gap']:
                ...
                print(rea_i)
                rea_i.bounds = (0, 0)
                solution = model_i_refined.optimize()
                if solution.objective_value > 0.001:
                    remove_list.add(rea_i.id)
                print('biomass: \t', model_i_refined.optimize().objective_value)
    ...
    for rea in remove_list:
        model_i_refined.reactions.get_by_id(rea).remove_from_model()
    ...
    model_i_refined = remove_sepcical_dup(model_i_refined)

    # case: general check duplication

    cofacters_set_1 = {'h2o_c', 'h2o_e', 'h_c', 'h_e'}
    print('\033[1;31;47m')
    check_df = My_def.model_refine.check_duplicate_rea(model_i_refined, cofacters_set_1, remove=False)

    check_id = list(check_df['id'])
    print('Duplicate reactions: ')
    for i in list(check_id):
        rea = model_i_refined.reactions.get_by_id(i)
    ...
    for k, v in keep_move_dic.items():
        model_i_refined = My_def.merge_model.merge_reactionsid(model_i_refined, k, v)

    model_i_refined.objective = "BIOMASS"
    print('Lreuteri_530 Biomass keep_move_dic:', Lreuteri_530.optimize())
    print('model_i_refined Biomass keep_move_dic:', model_i_refined.optimize())
    ...
    for i in model_i_refined.reactions:
        if i.bounds == (0.0, 0.0):
    ...
    for rea in check_bounds_set:
        try:
            model_i_refined.reactions.get_by_id(rea).bounds = Lreuteri_530.reactions.get_by_id(rea).bounds
        except:
            model_i_refined.reactions.get_by_id(rea).remove_from_model()

    model_i_refined.objective = "BIOMASS"
    print('Lreuteri_530 Biomass:', model_i_refined.optimize())

    # %% <general step: add _missing tag remove remove_useless_mets and gens >

    model_i_refined = My_def.model_refine.remove_useless_mets(model_i_refined)
    model_i_refined = My_def.model_refine.remove_useless_genes(model_i_refined)
    ...
    for met_i in model_i_refined.metabolites:
        ...
        model_i_refined.metabolites.get_by_id(met_i.id).annotation = annotation_i
    ...
    for rea_i in model_i_refined.reactions:
        if 'from' in rea_i.notes.keys():
            if 'model_i_from_Lreuteri_530' in rea_i.notes['from']:
                model_i_refined.reactions.get_by_id(rea_i.id).notes['from'].remove('model_i_from_Lreuteri_530')
            if 'transport' in rea_i.notes['from']:
                model_i_refined.reactions.get_by_id(rea_i.id).notes['from'].remove('transport')
                model_i_refined.reactions.get_by_id(rea_i.id).notes['from'].append('transport_reaction')
            if 'EX' in rea_i.notes['from']:
                model_i_refined.reactions.get_by_id(rea_i.id).notes['from'].remove('EX')
                model_i_refined.reactions.get_by_id(rea_i.id).notes['from'].append('exchange_reaction')
        else:
            notes_temp = set([])
            if rea_i.id in iML1515_reas:
            ...
            model_i_refined.reactions.get_by_id(rea_i.id).notes = notes_temp

    # %% <save >
    model_i_refined.objective = "BIOMASS"
    print('model_i_refined Biomass:', model_i_refined.optimize())

    for i in model_i_refined.metabolites:
        if i.compartment not in ['c', 'e']:
            i.compartment = i.id.split('_')[-1]

    cobra.io.save_json_model(model_i_refined, 'GEM_from_templates_' + sp_name + '_refined.json', sort='True')

    # My_def.io_file.model2txt(model_i_refined, 'GEM_from_templates_'+sp_name+'_draft.txt', sort='True')

    # comd = ' memote report snapshot --filename "model_i_refined_part01.html" model_i_refined_part01.xml'
    # os.system(comd)
