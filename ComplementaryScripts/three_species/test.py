...
def carbon_biomass(data_df, co2_produce):
    corbon_biomass = 6 * (data_df['fru'].values[0] - data_df['fru'].values[-1]) + \
                     2 * (data_df['ac'].values[0] - data_df['ac'].values[-1]) + \
                     1 * (data_df['for'].values[0] - data_df['for'].values[-1]) + \
                     4 * (data_df['but'].values[0] - data_df['but'].values[-1]) + \
                     3 * (data_df['lac'].values[0] - data_df['lac'].values[-1]) - co2_produce

    print(corbon_biomass)
    return corbon_biomass
...
co2_produce = 59.3
RI_carbon_biomass = carbon_biomass(data_df,co2_produce)
cell_number = np.ptp(data_df['R.i'])/(1e8)


data_df = FP_experimet_data
co2_produce = 17.49
FP_carbon_biomass = carbon_biomass(data_df,co2_produce)
cell_number = np.ptp(data_df['F.p'])/(1e8)


data_df = BH_experimet_data
co2_produce = 20
BH_carbon_biomass = carbon_biomass(data_df,co2_produce)
cell_number = np.ptp(data_df['B.h'])/(1e8)


data_df = RI_BH_experimet_data
co2_produce = 77.86390008
RI_BH_carbon_biomass = carbon_biomass(data_df,co2_produce)
cell_number = (np.ptp(data_df['R.i'])+ np.ptp(data_df['B.h']))/(1e8)


data_df = RI_FP_experimet_data
co2_produce = 82.82601914
RI_FP_carbon_biomass = carbon_biomass(data_df,co2_produce)
cell_number = np.ptp(data_df['F.p'])/(1e8)


data_df = FP_BH_experimet_data
co2_produce = 78
FP_BH_carbon_biomass = carbon_biomass(data_df,co2_produce)
cell_number = np.ptp(data_df['F.p'])/(1e8)'''
...
# 0.877*5
# iML1515.reactions.get_by_id('')
RI_experimet_data['R.i'] = RI_experimet_data['R.i'] * 0.877 * 5 / 71e8
...
experiment_data_df['R.i'] = experiment_data_df['R.i'] / (0.877 * 5 / 71)
simulation = sol
simulation[:, 1] = simulation[:, 1] / (0.877 * 5 / 71)
