...
iML1515_2.reactions.get_by_id('EX_o2_e').bounds = (-10, 1000.0)
...
for met_i in columns:
    if met_i != 'BIOMASS_Ec_iML1515_core_75p37M':
        ax.plot(x, abs(result[met_i]), label=label[met_i], linewidth=1)
ax.set_xlabel('Biomass rate ', fontsize=13)
ax.set_ylabel('Metabolites rates ', fontsize=13)
# ax.set_yticklabels( fontsize=18)
ax.legend(fontsize=12)
if iML1515_2.reactions.get_by_id('EX_o2_e').lower_bound == 0:
    ax.set_title('$E.coli$ Anaerobic', fontsize=15)
else:
    ax.set_title('$E.coli$ Aerobic', fontsize=15)
fig.show()
...
for met_i in columns:
    if met_i != 'BIOMASS_Ec_iML1515_core_75p37M':
        ax.plot(x, abs(result[met_i] / result['EX_glc__D_e']), 'o', label=label[met_i])
ax.set_xlabel('Biomass/Glucose yield ', fontsize=13)
ax.set_ylabel('Metabolites/Glucose yield ', fontsize=13)
# ax.set_yticklabels( fontsize=18)
ax.legend(fontsize=12)
if iML1515_2.reactions.get_by_id('EX_o2_e').lower_bound == 0:
    ax.set_title('$E.coli$ Anaerobic', fontsize=15)
